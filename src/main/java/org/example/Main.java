...
public class Main {
    ...
    //장바구니에 상품 있는지 확인
    private static void isemptyCart() {
        if (!menuData.getCart().isEmpty()) {
            ShowOrderMenu();
        } else {
    ...
    private static void WatingNumber() {
        ...
        menuData.allOrder(cart);                //총 판매 목록 저장
        int price = menuData.getTotalPrice();
        menuData.setAllTotalPrice(price);   //총 판매 금액 저장
